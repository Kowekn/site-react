...
[   
 ...
 [`Qual a diferença entre a versão sem registro e a versão registrada?`,
    `A diferença está no uso de algumas características. 
    ...
    O restante das funcionalidades são idênticas, tanto na parte do visualizador, como para gravação de arquivos VRX ou HTML. `],
 /*2 */   
 [`É necessário adquirir uma licença para cada usuário que for usar o componente?`,
    `A licença de uso do Visual ReportX é do tipo "Runtime Royalty Free", ou seja, todos os arquivos binários do componente 
    (Arquivos OCX e DLL) podem ser distribuídos para quantos usuários forem necessários sem nenhum custo. O registro, 
    quando realizado, precisa ser feito apenas nas máquinas de desenvolvimento. `],
 /*3 */   
 [`O Visual ReportX pode ser usado com o VB5?`,
    `Sim. Basta instalar os arquivos de RunTime do VB6-SP6 que podem ser baixados no site da Microsoft. 
    Consulte o artigo `,<a id="a" href="http://support.microsoft.com/kb/290887/pt-br" target="_blank">KB290887</a>,` e veja 
    como baixar os arquivos de runtime.  `],
 /*4 */   
 [`O Visual ReportX pode ser usado com o VB.NET?`,
    `Não. O VB.NET é uma nova plataforma de desenvolvimento. 
    ...
    na nova versão .NET sem sofrer ajustes.  `],
 /*5 */   
 [`Como fazer para adquirir a versão registrada?`,
    `Visite a página de `, <Link id="a" to="/registro">registro</Link>,` para saber mais sobre a versão registrada. `],
 /*6 */   
 [`Como exportar o relatório para o formato PDF?`,
    `O componente não faz a exportação diretamente, mas você pode instalar um driver virtual de impressora que faça isso. 
    ...
    e será aberta uma janela pedindo o nome do arquivo a ser gerado. `],
 /*7 */   
 [`Quando o relatório é chamado apresenta o erro "Erro 401 - Application defined or object defined error" 
    ...
    `Esse erro provavelmente se deve ao fato do usuário estar tentando apresentar um relatório a partir de um 
    ...
    voltar o form Modal para a tela do aplicativo.<br> Me.Hide<br> frmRelatorio.Config<br> Me.Show vbModal<br>  `],
 /*8 */   
 [`Não é possível colocar objetos em posições além da largura do formulário do VB.`,
    `O componente usa os recursos da IDE do Visual Basic para o desenho do relatório, mas o formulário em modo de design 
    ...
    linhas de grade sejam mostradas nas seções. `],
 /*9 */   
 [`Quando o "Zoom" é ativado para 75% ou 50% os textos estão sendo cortados. Isso é normal?`,
    `Para que o "Zoom" funcione corretamente na hora da vizualização deve-se sempre utilizar fontes "True Type" 
    durante o design do relatório. O VB usa a fonte "MS Sans Serif" como padrão nos formulários e essa fonte não é uma fonte 
    "True Type". Tente fontes como "Arial" ou "Times New Roman".  `],
 /*10 */  
  [`Como fazer para colocar o número de página no relatório?`,
    `Para colocar o número de página no relatório, coloque um objeto "ReportField" com a propriedade 
    ...
    Para maiores informações consulte a ajuda do componente ou o projeto de exemplo que acompanha o pacote. `],
 /*11 */  
  [`Existe algum tutorial ou manual do Visual ReportX?`,
    `A documentação oficial do componente é o Arquivo de Ajuda (ReportX.HLP) que é instalado com o componente. 
    ...
    pacote também contém muita informação através dos relatórios de exemplo.  `],
 /*12 */  
  [`O Visual ReportX trabalha com sub-relatórios?`,`O Visual ReportX não tem uma seção para sub-relatórios mas tem as 
    ...
    e obter dois tipos de relatórios. No exemplo que acompanha o pacote existe um relatório do tipo "dois em um".  `],
 /*13 */  
  [`O relatório está sendo cancelado abruptamente com um erro "Disk Full". O que está acontecendo?`,
    `A partir da versão 1.1.43 o componente passou a criar arquivos temporários no diretório temporário do Windows. 
    ...
    Deve-se então liberar espaço no diretório temporário do usuário.  `],
 /*14 */  
  [`Em alguns relatórios um campo que está na seção de cabeçalho ou rodapé não está aparecendo na última página. 
    Qual é o problema?`,
    `Apenas os campos que estiverem na seção de "Detalhe" ou "Cabeçalho de Grupo" podem ser impressos diretamente do recordset 
 ...
 /*15 */  
  [`Como fazer para visualizar o arquivos .VRX gerados pelo Visual ReportX?`,
    `Se você já instalou o pacote do Visual ReportX, basta um click duplo sobre o arquivo no Windows Explorer para abrí-lo. 
    ...
    funcionamento correto do visualizador. `],
 /*16 */  
  [`Quando um novo relatório está sendo criado exibe a mensagem "Erro 438 - Object doesn't support this property or method" 
    ...
    `Esse erro estava sendo apresentado pela versão 1.2.56 devido a um problema na rotina de inicialização do objeto. 
    O problema foi corrigido na versão 1.2.57, basta desinstalar o componente, fazer o download e instalar a versão mais atual.  `],
 /*17 */  
  [`Na hora de gerar o Setup com o Setup Wizard está sendo aparecendo a mensagem "Visual ReportX version information is out of date". O que fazer?`,
    `A partir da versão 1.1.46 o pacote de instalação distribuiu o arquivo ReportX.DEP que contém as informações de dependência e de versão e 
    ...
    de acordo com a versão do componente. `],
 /*18 */  
  [`Quando é colocada uma imagem no relatório, ao imprimir ela sai em tamanho menor do que quando estava na tela. O que fazer?`,
    `Para imprimir imagens use um objeto "Image" e coloque a propriedade Stretch=True. Se quiser usar um objeto "PictureBox" coloque a propriedade 
    Tag=STRETCH para que ele tenha o mesmo comportamento do objeto Image. `],
 /*19 */  
  [`Ao enviar o relatório para XLS ou HTML está apresentando um erro.`,
    `Verifique se ao instalar o componente está enviando e registrando, junto com o arquivo ReportX.OCX o arquivo RepXHTM.DLL. 
    Os dois precisam estar registrados na máquina onde roda o relatório para as funcionalidades de exportação funcionarem adequadamente. `],
 /*20 */  
  [`Utilizo uma versão anterior do Visual ReportX e após atualizar, os projetos não abrem no ambiente do Visual Basic. Existe incompatibilidade?`,
    `Os componentes ActiveX como o VRX, gravam nos arquivos VBP e FRM, informações sobre os componentes utilizados nos projetos e a versão de interface 
...
const updatedPerguntas = perguntas?.map((item,index) => {
   if(index === 4 || index === 14 || index === 19 || index === 5 || index == 2){
      return(
         <div class="">
...
const [InputPesquisaCSS, setPInputPesquisaCSS] = useState(``)
    ...
    return(
            ...
            <div className={`flex flex-col  p-4  pl-8  pb-16  text-start `}>
                <div>
                  ...
                  <input type="text"   className={`relative sticky top-0  `}
                ></input>
                    {updatedPerguntas}
                </div>
                <div>
